...
namespace Smath {
	...
	Matrix4::Matrix4(
		...
		matrix[0] = Vector4(x1, y1, z1, w1);
		matrix[1] = Vector4(x2, y2, z2, w2);
		matrix[2] = Vector4(x3, y3, z3, w3);
		matrix[3] = Vector4(x4, y4, z4, w4);
	}

	Matrix4::Matrix4(const Matrix3& mat) {
		matrix[0] = Vector4(mat[0], 0.0);
		matrix[1] = Vector4(mat[1], 0.0);
		matrix[2] = Vector4(mat[2], 0.0);
		matrix[3] = Vector4(0.0, 0.0, 0.0, 1.0);
	}

	Matrix4::Matrix4(const real_t diag) {
		matrix[0] = Vector4(diag, 0, 0, 0);
		matrix[1] = Vector4(0, diag, 0, 0);
		matrix[2] = Vector4(0, 0, diag, 0);
		matrix[3] = Vector4(0, 0, 0, diag);
	}
	...
	void Matrix4::operator *=(const Matrix4& other) {
		Vector4 r1(matrix[0].x, matrix[1].x, matrix[2].x, matrix[3].x);
		Vector4 r2(matrix[0].y, matrix[1].y, matrix[2].y, matrix[3].y);
		Vector4 r3(matrix[0].z, matrix[1].z, matrix[2].z, matrix[3].z);
		Vector4 r4(matrix[0].w, matrix[1].w, matrix[2].w, matrix[3].w);
		const Vector4& c1 = other.matrix[0];
		const Vector4& c2 = other.matrix[1];
		const Vector4& c3 = other.matrix[2];
		const Vector4& c4 = other.matrix[3];
	...
	Vector4 Matrix4::operator*(const Vector4& vec) {
		...
		return Vector4(dot(r1, vec), dot(r2, vec), dot(r3, vec), dot(r4, vec));
	}

	Vector4& Matrix4::operator[](short index) {
		if (index < 0 || 3 < index) {
