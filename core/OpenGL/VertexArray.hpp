...
namespace ScOpenGL {
	...
	class VertexArray {
		...
		unsigned int id;
		std::unordered_map<GLenum, ScOpenGL::Buffer*> boundBuffers;
		std::unordered_map<BufferIndex_t, ScOpenGL::Buffer*> vertexBuffers;
		...
		void bindBuffer(ScOpenGL::Buffer& buffer, GLenum bufferType);

		bool boundBuffersHasBufferType(GLenum bufferType);

		inline auto findVertexBuffer(ScOpenGL::Buffer& buffer) {
	...
	public:
		...
		void bindIndexBuffer(ScOpenGL::Buffer& buffer);
		void bindVertexBuffer(ScOpenGL::Buffer& buffer, unsigned int bufferIndex = 0);
		void unbindVertexBuffer(ScOpenGL::Buffer& buffer);
